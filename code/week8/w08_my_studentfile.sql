...
SELECT concat('The store ', store_id, ' email is ', email) AS'Store Email'
FROM store;
...
SELECT LOCATE('Girl', product_name) AS 'locate'
FROM product
...
SELECT SUBSTRING(product_name, LOCATE('Girl', product_name), 4) AS 'substring'
FROM product
WHERE product_name LIKE '%Girl%';
